...
class CVThread(threading.Thread):
    ...
    def servoMove(ID, Dir, errorInput):
        # Reuse of Adeept
        # control servo at camera move up/down
        if ID == 1:
    ...
    def dcMotorMove(calCenter):
        """
            calCenter <0 => object is on the left
            calCenter >0 => object is on the right

        """
        CVThread.motors.updateE(calCenter)  # keep track of the error
        if abs(calCenter) > 50:
            if calCenter < 0:
    ...
    def objSpacing():
        # Keeping constant distant of ~18-20cm
